...
<div class="container-fluid">
    <div class="page-header">
        <h1>电信分院毕业生选题系统<span class="label label-success">管理学生信息</span>页面
    ...
    <div id="collapse2" class="panel-collapse collapse in" role="tabpane2" aria-labelledby="heading2">
        ...
        <div class="col-sm-10">
            <table class="table table-hover table-striped table-condensed">
                ...
                <tr>
                    <td colspan="2">
                        <a href="<?php echo U('Stu/add');?>" class="btn btn-danger">添加学生</a>
                    </td>
                    <td colspan="8" class="text-right">
                        <form class="form-inline" method="post" action="<?php echo U('Stu/search');?>" onsubmit="return chkNull()">
                            <div class="form-group">
                                <label for="stunum">请输入 <span class="badge">学号</span></label>
                                <input type="text" id="stunum" name="stunum" class="form-control" value="<?php echo ($sn); ?>"
                                       placeholder="请输入学号">
                            </div>
                            <div class="form-group">
                                <label for="stuname">或 <span class="badge">姓名</span></label>
                                <input type="text" id="stuname" name="stuname" class="form-control" value="<?php echo ($snm); ?>"
                                       placeholder="请输入姓名">
                            </div>
                ...
                <?php if(is_array($result)): $i = 0; $__LIST__ = $result;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="text-center">
                        <td><input type="checkbox" id="selItem" name="selItem" value="<?php echo ($vo["s_id"]); ?>" onclick="itemClk()">
                        </td>
                        <td><?php echo ($i); ?></td>
                        <td><?php echo ($vo["s_num"]); ?></td>
                        <td><?php echo ($vo["s_name"]); ?></td>
                        <td><?php echo ($vo["s_pwd"]); ?></td>
                        ...
                        <td>
                            <a href="<?php echo U('Stu/modify');?>?sid=<?php echo ($vo["s_id"]); ?>&act=<?php echo ($act); ?>&sn=<?php echo ($sn); ?>&snm=<?php echo ($snm); ?>&p=<?php echo $_GET['p'];?>">
                                <span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>
                            </a>&nbsp;
                            <a href="<?php echo U('Stu/delete');?>?sid=<?php echo ($vo["s_id"]); ?>&act=<?php echo ($act); ?>&sn=<?php echo ($sn); ?>&snm=<?php echo ($snm); ?>&p=<?php echo $_GET['p'];?>">
                ...
                <tr>
                    <td colspan="3">
                        <form method="post"
                              action="<?php echo U('Stu/delete');?>?sid=<?php echo ($vo["s_id"]); ?>&act=<?php echo ($act); ?>&sn=<?php echo ($sn); ?>&snm=<?php echo ($snm); ?>&p=<?php echo $_GET['p'];?>"
